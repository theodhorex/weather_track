...
def build_message(status: str, weather: dict) -> str:
    ...
    if status == _STATUS_RAIN:
        ...
        return (
            f"🌧️ <b>RAIN ALERT - {weather_main}</b>\n\n"
            f"📊 <b>Weather Details:</b>\n"
            f"   • Rain Probability: <b>{rain_pop:.1f}%</b> (threshold {threshold:.0f}%)\n"
            f"   • Condition: {weather_main}\n"
            f"   • Temperature: {temp_str}\n"
            f"   • Humidity: {humidity_str}\n"
            f"   • Measurement Time: {ts_str}\n\n"
            f"💡 <b>Recommendation:</b>\n"
            f"   Prepare your umbrella and don't forget to wear a raincoat for outdoor activities.\n"
            f"   Be aware of potential light flooding in low-lying areas, traffic may be affected.\n\n"
            f"🌦️ <i>Weather will continue to be monitored for further changes.</i>"
        )
    else:
        temp_str = f"{temp:.1f}°C" if temp is not None else "N/A"
        humidity_str = f"{humidity}%" if humidity is not None else "N/A"
        ...
        return (
            ...
            f"   • Measurement Time: {ts_str}\n\n"
            f"💡 <b>Information:</b>\n"
            f"   Weather conditions are currently good for outdoor activities.\n"
            f"   Remain alert as weather can change rapidly.\n\n"
            f"🌦️ <i>The system will continue to monitor and provide alerts when needed.</i>"
        )
